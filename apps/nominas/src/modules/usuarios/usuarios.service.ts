...
export class UsuariosService implements IUserService {
  ...
  /**
   * IUserService contract â€” used by AuthService for login.
  ...
  /**
   * IUserService contract ï¿½ used by AuthService.register for persistence.
  ...
  async create(createDto: CreateUsuarioDto): Promise<UsuarioPublic> {
    ...
    if (existing) {
      // PR5 / M3 / REQ-usuarios-3 â€” return the same public shape and
  ...
  /**
   * M2 / hardening-medium-low â€” paginated retrieval.
   */
  ...
  async update(
    ...
    // L10 / hardening-medium-low â€” type-safe update shape. Only the
  ...
  /**
   * PR5 / H1 â€” read the user's roles by id. Used by the audited role
  ...
  /**
   * PR5 / H1 â€” atomic role update; validates the allow-list at the DTO
